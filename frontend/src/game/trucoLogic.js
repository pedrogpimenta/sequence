...
export function trucoRank(c) {
  const v = cardNum(c), s = cardSuit(c)
...
export const SUIT_COLORS  = {
  ...
  O: { bg: 'linear-gradient(145deg,#1e1700,#3d3000)', border: '#7a6200', text: '#ffe97a' },
  C: { bg: 'linear-gradient(145deg,#12002b,#2a005e)', border: '#5c008a', text: '#e088ff' },
}

export const MACHO = new Set(['1E', '1B', '7E', '7O'])
export const isMacho = c => MACHO.has(c)

export const envidoVal = c => { const v = cardNum(c); return v >= 10 ? 0 : v }

export function calcEnvido(hand) {
  const bySuit = {}
  for (const c of hand) {
    if (!c) continue
    const s = cardSuit(c)
...
export const ENVIDO_LEVEL_NAMES = ['', 'Envido', 'Envido Envido', 'Real Envido', 'Falta Envido']
export const TRUCO_LEVEL_NAMES  = ['', 'Truco', 'Retruco', 'Vale Cuatro']

// ── Full game logic (mirrors server) ─────────────────────────────────────────
